...
当然React的右移算法和Vue3的的双端比较+lis算法比起来几乎是被完爆的，如abcd->dabc，React的diff算法会傻傻地移动三个节点，而Vue3的算法则能符合预期地只移动一个节点。<br>
不过这个仓库的目的是学习React而不是研究React的缺陷，所以相关内容不再赘述，感兴趣的可以看这篇文章（还没写，先挖个坑）。
